...
public:
    ...
    inline const node_data& data_of(const GraphT::node& node) const;
    inline node_data& data_of(const GraphT::node& node);
...
template<typename T, class WeightFunc = unsigned(*)(const T&, const T&), class HeuristicFunc = int>
path aStar(
